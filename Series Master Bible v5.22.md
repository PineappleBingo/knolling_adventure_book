...
* **Typography Assets (Google Fonts):**
    ...
    * `FONT_SUBTITLE`: **"Fredoka-Regular.ttf"** (Note: File is Fredoka-Regular.ttf, not FredokaOne)
...
* **Core Python Libs:**
    ...
    * `google-generativeai` (Strictly for Text/QA/Vision Analysis via Gemini) **⚠️ DEPRECATED - See note below**
    ...
    * `oauth2client` (Google Sheets authentication)
* **Standard Library Imports:**
    * `asyncio` (Async orchestration for Telegram bot)
    * `uuid` (Run ID generation)
    * `datetime` (Timestamp tracking)
    * `json` (API payload construction)
    * `base64` (Image decoding from API responses)
    * `glob` (Asset file pattern matching)
    * `logging`, `time`, `os` (Standard utilities)
* **Assets Structure:**
...
* **Environment Variables (.env):**
    * `GOOGLE_API_KEY` (Required for all AI operations)
    * `TELEGRAM_TOKEN` (Required for bot authentication)
    * `DEPLOYMENT_TIER` ("FREE" or "PAID" - controls model selection and rate limits)
    * `PAGE_COUNT` (Optional - default 50)
    * `TARGET_PAGES` (Optional - specific pages to generate, e.g., "1,50")
* **Library Deprecation Alert:**
    * `google-generativeai` is currently in use but **DEPRECATED** as of January 2026.
    * Google recommends migrating to `google.genai` package.
    * Current implementation: `prompt_generator.py` and `qa_agent.py` use deprecated library.
    * Future Action Required: Migrate to `google.genai` in next major version.
...
* **Tasks:**
    ...
    * **Font Loader:** Registers all `.ttf` files defined in **Section 0.0** with `reportlab` at startup. Implementation includes graceful degradation: if a font file is missing, logs warning but continues execution. Implementation: `pdf_assembler.py` lines 28-47.
...
* **Tasks:**
    * **Discovery (Smart Filtering with Fallback):**
        * **Primary Pattern:** `assets/ref_{asset_type}_01.png` (Strict filtering for master style references)
        * **Fallback Logic:** If primary not found, search `assets/ref_{asset_type}_*.png` excluding files with `_structure`, `_wireframe`, `_layout`, or `_kdp` in filename
        * **Implementation:** `prompt_generator.py` lines 55-60
...
* **Mission:** Manage the API Loop via REST (Imagen 4.0 or Gemini Flash).
* **Tasks:**
    * **Dual-Tier Implementation:**
        * **PAID Tier:** Uses Imagen 4.0 via `:predict` endpoint
        ...
        * **FREE Tier:** Uses Gemini 2.0 Flash Exp via `:generateContent` endpoint
            * Endpoint: `https://generativelanguage.googleapis.com/v1beta/{GEN_MODEL_ID}:generateContent`
            * Payload: `{"contents": [{"parts": [{"text": "..."}]}]}`
            * Response: `{"candidates": [{"content": {"parts": [{"inlineData": {"data": "..."}}]}}]}`
        * **Implementation:** `image_generator.py` lines 39-115
